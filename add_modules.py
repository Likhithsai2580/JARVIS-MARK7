...
class ConversationHandler:
    ...
    def handle_conversation(self, user_prompt: str):
        ...
        try:
            return self._process_conversation(user_prompt)
        except Exception as e:
            logger.error(f"ERROR: {str(e)}")
            return f"Error occurred: {str(e)}"

    def _process_conversation(self, user_prompt: str):
        # Classify the query to determine which model to use
        classification_prompt = dedent(
            f"""\
            Classify the following query into one of the categories: 'dmm', 'edmm', 'ndmm':
            Query: {user_prompt}
            Categories:
            - 'dmm': Queries related to data manipulation and management.
            - 'edmm': Queries related to emotional data manipulation and management.
            - 'ndmm': Queries related to note data manipulation and management.
            """
        )
        classification_response = chatBotLLM.run(classification_prompt)
        classification = classification_response.strip().lower()

        if classification == "dmm":
            result = dmm(user_prompt)
        elif classification == "edmm":
            result = edmm(user_prompt)
        elif classification == "ndmm":
            result = ndmm(user_prompt)
        else:
            result = []
